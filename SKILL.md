...
description: Navigate Eleme (饿了么) food delivery with smart restaurant selection, deal finding, ordering strategies, and browser-based delivery/location checks.
metadata:
  clawdbot:
    emoji: "🍜"
    requires:
...
When the task involves live Eleme pages, follow the shared **browser-commerce-base** workflow:
- restaurant discovery and public browsing → `openclaw`
- account-specific red packets / orders / saved addresses → `user` only when needed
- treat location, ETA, delivery fee, and minimum order as first-class extracted fields
- re-snapshot after changing address, filters, or restaurant tabs

Key browser extraction order on Eleme:
- 餐厅名
- 评分 / 月销量
- 起送价 / 配送费 / 配送时长
- 满减 / 红包 / 平台优惠
